...
import { applyEnv, makeExtEnv, Env, Store, setStore, extendStore, ExtEnv, theGlobalEnv, globalEnvAddBinding, theStore, applyExtEnv, applyStore } from "./L21-env-store"; //applyEnvStore
...
//                     isVarRef(exp) ? bind(applyEnv(env, exp.var), (num: number) => applyStore(theStore, num)) :
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses: number[] = map((val: Value) => { extendStore(theStore, val); return theStore.vals.length - 1 }, args)
...
const evalDefineExps = (def: DefineExp, exps: Exp[]): Result<Value> =>
    bind(applicativeEval(def.val, theGlobalEnv),
        (value: Value): Result<Value> => {
            globalEnvAddBinding(def.var.var, extendStore(theStore, value).vals.length - 1)
            return evalSequence(exps, theGlobalEnv)
        })


const evalSet = (setExp: SetExp, env: Env): Result<void> =>
    safe2((addr: number, v: Value) => makeOk(setStore(theStore, addr, v)))
        (applyEnv(env, setExp.var.var), applicativeEval(setExp.val, env))
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vArray: Value[]) => {
        const addresses = map((v: Value) => { extendStore(theStore, v); return theStore.vals.length - 1 }, vArray)
