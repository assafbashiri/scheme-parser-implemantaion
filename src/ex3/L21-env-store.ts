...
export const isStore = (s:any): Boolean=>s.tag==="Store";
...
export const applyStore = (store: Store, address: number): Result<Value> => {
    return (address < store.vals.length && address >= 0) ? makeOk(unbox(store.vals[address])) :
        makeFailure("The address is not valid")
}
...
export const isInStore = (val:Value): number =>{
    const box:Box<Value> | undefined = theStore.vals.find((b:Box<Value>)=>unbox(b)===val)
    return box===undefined ? -1 : theStore.vals.indexOf(box)
}
// ========================================================
...
export const applyGlobalEnv = (env: GlobalEnv, v: string): Result<number> =>
    unbox(env.vars).includes(v) ? makeOk(unbox(env.addresses)[unbox(env.vars).indexOf(v)]) :
        makeFailure("There is no such element")
