...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | SetExp; //start //finish
        ...
        op === "if" ? parseIfExp(params) :
            op === "lambda" ? parseProcExp(first(params), rest(params)) :
                op === "let" ? parseLetExp(first(params), rest(params)) :
                    op === "set!" ? parseSetExp(params) :   //change
                        op;
...
//change
const unparseSetExp = (s: SetExp): string =>
    `(set! ${s.var.var} ${unparse(s.val)})`
//stop
                                        ...
                                        isLetExp(exp) ? unparseLetExp(exp) :
                                            isDefineExp(exp) ? `(define ${exp.var.var} ${unparse(exp.val)})` :
                                                isProgram(exp) ? `(L21 ${unparseLExps(exp.exps)})` :
                                                    isSetExp(exp) ? unparseSetExp(exp) : //change
                                                        exp;